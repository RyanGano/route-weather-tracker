...
function restrictionBadge(restriction: TravelRestriction) {
  switch (restriction) {
    ...
    case TravelRestriction.ChainsRequired:
      return <Badge bg="warning" text="dark">Chains Required</Badge>;
    case TravelRestriction.TiresOrTraction:
      return <Badge bg="warning" text="dark">Tires/Traction</Badge>;
    default:
...
function conditionBadge(condition: string | undefined) {
  ...
  if (lower.includes('bare') || lower.includes('dry')) {
    return <Badge bg="success" className="ms-1">{condition}</Badge>;
  }
  if (lower.includes('snow') || lower.includes('ice') || lower.includes('slush')) {
    return <Badge bg="warning" text="dark" className="ms-1">{condition}</Badge>;
  }
  return <Badge bg="secondary" className="ms-1">{condition}</Badge>;
}
...
export default function PassCard({ pass }: PassCardProps) {
  ...
  return (
    <Card className="mb-4 shadow-sm">
      ...
      {hasRestriction && (
        <Alert variant="warning" className="mb-0 rounded-0 py-2 px-3 border-0 border-bottom">
          <strong>Travel Restrictions:</strong>{' '}
          EB: {restrictionBadge(condition!.eastboundRestriction)}{' '}
          WB: {restrictionBadge(condition!.westboundRestriction)}
        </Alert>
      )}
      ...
      <Card.Body>
        <div className="row g-3">
          ...
          <div className="col-12 col-lg-6">
            <h6 className="text-muted text-uppercase fw-semibold mb-2" style={{ fontSize: '0.7rem', letterSpacing: '0.08em' }}>Live Camera</h6>
            <WebcamViewer cameras={cameras} />
          </div>

          {/* Weather column */}
          <div className="col-12 col-lg-6">
            <h6 className="text-muted text-uppercase fw-semibold mb-2" style={{ fontSize: '0.7rem', letterSpacing: '0.08em' }}>Weather</h6>
