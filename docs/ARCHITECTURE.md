...
- **Static registries** (`Data/`) are the source of truth for geography: `PassRegistry` (every pass with coords/elevation/highway), `RouteRegistry` (highways), `RouteEndpointRegistry` (cities, including `RoutingHubs` used to bias route computation). Pass coordinate/elevation accuracy and data-source coverage are documented in [`docs/passes.md`](passes.md) and machine-checked by `tools/validate_passes.py` → [`docs/passes-validation.md`](passes-validation.md).
